  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="日期" prop="createdAt" width="180">
            ...
            <el-table-column align="left" label="用户名称" prop="userName" width="120" />
    ...
    <el-input v-model="formData.userName" :clearable="true" placeholder="请输入用户名称" />
</el-form-item>
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteUserWalletByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateUserWalletFunc = async(row) => {
...
// 删除行
const deleteUserWalletFunc = async (row) => {
    const res = await deleteUserWallet({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     ...
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return btnLoading.value = false
              ...
              switch (type.value) {
                case 'create':
                  res = await createUserWallet(formData.value)
                  break
                case 'update':
                  res = await updateUserWallet(formData.value)
                  break
                default:
                  res = await createUserWallet(formData.value)
                  break
