  ...
  <div>
    <div class="gva-search-box">
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline" :rules="searchRule"
        ...
        <el-form-item>
          <el-input :maxlength="60" v-model="searchInfo.value"  placeholder="用户名或手机号码"></el-input>
        </el-form-item>
    ...
    <div class="gva-table-box">
      <el-table ref="multipleTable" style="width: 100%" tooltip-effect="dark" :data="tableData" row-key="ID"
        ...
        <el-table-column align="left" label="手机号码" prop="phone" width="120" />
        <el-table-column align="left" label="uid字段" prop="uid" width="400" />
        <el-table-column align="left" label="用户名" prop="name" width="120" />
        <el-table-column align="left" label="体验截止时间" prop="vipExpireTime" width="180">
          <template #default="scope">{{ formatDate(scope.row.vipExpireTime) }}</template>
        </el-table-column>
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
          <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button"
              @click="updateUsersFunc(scope.row)">变更</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-drawer destroy-on-close size="800" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
      ...
      <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="体验截止时间:" prop="vipExpireTime">
          <el-date-picker v-model="formData.vipExpireTime" type="date" style="width:100%" placeholder="选择日期"
            :clearable="true" />
        </el-form-item>
      </el-form>
    </el-drawer>
...
const updateUsersFunc = async (row) => {
  const res = await findUsers({ UID: row.uid })
  type.value = 'update'
  if (res.code === 0) {
