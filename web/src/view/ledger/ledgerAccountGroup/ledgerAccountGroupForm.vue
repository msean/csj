...
<template>
  <div>
    ...
    <RichEdit v-model="formData.accountGroup"/>
</el-form-item>
        <el-form-item label="tilte:" prop="title">
    <el-input v-model="formData.title" :clearable="true" placeholder="请输入tilte" />
</el-form-item>
        <el-form-item>
...
const init = async () => {
 // 建议通过url传参获取目标数据ID 调用 find方法进行查询数据操作 从而决定本页面是create还是update 以下为id作为url参数示例
    if (route.query.id) {
      const res = await findLedgerAccountGroup({ ID: route.query.id })
      if (res.code === 0) {
