  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
            ...
            <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
                ...
                <el-button  type="primary" link icon="edit" class="table-button" @click="updateLedgerPermissionFunc(scope.row)">编辑</el-button>
    ...
    >
      ...
      >
        ...
        <el-form-item label="允许操作用户" prop="oprUsers">
          <el-input
            v-model="formData.oprUsers"
            :clearable="true"
            placeholder="请输入允许的操作用户多个用,(英文状态)分隔"
          />
        </el-form-item>
      </el-form>
    </el-drawer>
    ...
    <el-drawer destroy-on-close :size="appStore.drawerSize" v-model="detailShow" :show-close="true" :before-close="closeDetailShow" title="查看">
        <el-descriptions :column="1" border>
            ...
            <el-descriptions-item label="群聊">
              {{ detailForm.chatGroupName }}
            </el-descriptions-item>
            <el-descriptions-item label="允许操作人">
                <RichView v-model="detailForm.oprUsers" />
            </el-descriptions-item>
          </el-descriptions>
    </el-drawer>
  </div>
...
<script setup>
import {
  createLedgerPermission,
  deleteLedgerPermission,
  deleteLedgerPermissionByIds,
  updateLedgerPermission,
  findLedgerPermission,
  getLedgerPermissionList
} from '@/api/usage/ledgerPermission'
// 富文本组件
...
const getTableData = async() => {
  const table = await getLedgerPermissionList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteLedgerPermissionFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteLedgerPermissionByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateLedgerPermissionFunc = async(row) => {
    // const res = await findLedgerPermission({ ID: row.ID })
    // type.value = 'update'
    // if (res.code === 0) {
...
// 删除行
const deleteLedgerPermissionFunc = async (row) => {
    const res = await deleteLedgerPermission({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     ...
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return btnLoading.value = false
              ...
              switch (type.value) {
                case 'create':
                  res = await createLedgerPermission(formData.value)
                  break
                case 'update':
                  res = await updateLedgerPermission(formData.value)
                  break
                default:
                  res = await createLedgerPermission(formData.value)
                  break
