...
func (usersService *UsersService) CreateUsers(users *biz.Users) (err error) {
...
func (usersService *UsersService) DeleteUsers(ID string) (err error) {
	err = global.GVA_DB.Delete(&biz.Users{}, "uid = ?", ID).Error
	return err
}
...
func (usersService *UsersService) DeleteUsersByIds(IDs []string) (err error) {
	err = global.GVA_DB.Delete(&[]biz.Users{}, "uid in ?", IDs).Error
	return err
}

// UpdateUsers 更新users表记录
// Author [piexlmax](https://github.com/piexlmax)
func (usersService *UsersService) UpdateUsers(users biz.Users) (err error) {
	err = global.GVA_DB.Model(&biz.Users{}).Where("uid = ?", users.Uid).Updates(&users).Error
	return err
}

// GetUsers 根据ID获取users表记录
// Author [piexlmax](https://github.com/piexlmax)
func (usersService *UsersService) GetUsers(UID string) (users biz.Users, err error) {
...
// Author [piexlmax](https://github.com/piexlmax)
func (usersService *UsersService) GetUsersInfoList(info biz_request.UsersSearch) (list []biz.Users, total int64, err error) {
	limit := info.PageSize
